<template>
  <img v-if="logo" :src="`/${logo}`" class="h-8 object-contain transition-transform duration-200 group-hover:scale-105" />

  <div v-else class="flex items-center">
    <div class="relative">
      <!-- Glow effect behind icon -->
      <div class="absolute inset-0 rounded-lg bg-indigo-500/30 blur-md"></div>
      <svg
        class="relative h-8 transition-all duration-200 group-hover:scale-110"
        style="color: #6875f5"
        fill="none"
        viewBox="0 0 34 32"
    ...
    <span
      class="bg-gradient-to-r from-white to-gray-200 bg-clip-text text-[1.375rem] font-bold text-transparent ltr:ml-2.5 rtl:mr-2.5 transition-all duration-200 group-hover:from-indigo-200 group-hover:to-white"
      >{{ name }}</span
    >
  </div>
</template>
